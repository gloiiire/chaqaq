...
@MainActor
@Suite("LeafViewModel — error handling")
struct VMErrorHandlingTests {
    ...
    @Test func loadOfDeletedLeafCapturesError() throws {
        let (api, url) = try makeApi(); defer { cleanup(url) }
        let leafId = try api.createLeaf(title: "To delete")
    ...
    @Test func loadOfRandomUuidCapturesError() throws {
        let (api, url) = try makeApi(); defer { cleanup(url) }
        let vm = LeafViewModel(leafId: UUID().uuidString, api: api)
        vm.load()
        #expect(vm.errorMessage != nil)
    }

    @Test func saveBlockOfMissingLeafCapturesError() throws {
        let (api, url) = try makeApi(); defer { cleanup(url) }
        let leafId = try api.createLeaf(title: "Doc")
        try api.deleteLeaf(id: leafId)
