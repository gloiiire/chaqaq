...
//   • Clicking the ↗ icon inside the Name cell opens LeafView.
//   • Subsequent columns are user-defined metadata properties.
//
// Used by `BookView` when the active view's type is `.table`. The
...
// Editable row name on the left + ↗ arrow to open the linked leaf.
