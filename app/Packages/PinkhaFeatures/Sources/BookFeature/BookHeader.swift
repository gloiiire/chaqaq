...
/// Doc-like header for a Book : cover banner (optional) + icon
...
public struct BookHeaderView: View {
    ...
    // Built-in cover gradients shared with leaves — keeps the
        ...
        VStack(alignment: .leading, spacing: 0) {
            if let coverId = vm.cover {
                coverBanner(coverId)
                    ...
                    // edge, matching the leaf cover behaviour.
            ...
            if !vm.locked {
                ...
                // title doesn't slide upward — matches the leaf
                // header treatment.
                Color.clear.frame(height: 50)
            }

            // Title + description. Locked books render plain Text —
            // not a disabled TextField — mirroring how leaves swap
    ...
    private func coverBanner(_ id: String) -> some View {
        if id.hasPrefix("http://") || id.hasPrefix("https://"),
           let url = URL(string: id) {
            ...
            // Local file in the covers directory (reuse the leaf
            // covers directory — they share storage).
            if let dir = try? LeafViewModel.coversDirectory(),
