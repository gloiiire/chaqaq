...
// Book — full payload with rows and views.

public struct BookFfi: Codable {
    public let id: String
    public let title: [InlineTextFfi]
    /// Optional banner image identifier — URL or local filename. Mirrors
    /// the leaf cover surface so the doc-like DB header can render
    ...
    /// At least one view is always present (Rust `Book::new` seeds a
    ...
    /// Read-only flag. Mirrors `Leaf.locked` ; defaults `false`
