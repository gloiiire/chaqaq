...
// ── Document publish-date picker ──────────────────────────────────────────────
//
// Surfaced from the editor's overflow menu : lets the user override the
// document's effective publish date (independent of the immutable
// creation timestamp) and reset it back to the default at any time.
// Mirrors the per-row PublishDatePickerSheet on the database list view
// so the UX vocabulary stays consistent across notes and DB rows.

struct DocumentPublishDateSheet: View {
        ...
        NavigationStack {
            Form {
                ...
                Section {
                    ...
                    Text("The created date stays as a fingerprint of when the document came into being. The publish date is what controls how the doc surfaces in date-sorted views.")
                }
