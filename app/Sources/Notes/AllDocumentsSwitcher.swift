...
/// menu (`More → All documents`). Lays the entire workspace out as a
...
struct AllDocumentsSwitcher: View {
    ...
    private var topBar: some View {
        HStack(spacing: 10) {
            if searching {
                ...
                // Center pill — Safari has the workspace/account picker
                // here. We surface the doc count + the implicit "All
                // documents" scope. Tapping does nothing for now (we
    ...
    private var grid: some View {
        ...
        SafariSwitcher(
            ...
            onSelect: { docId in onSelect(docId) },
            onClose: { docId in tabManager.close(docId: docId) }
        )
    }
    ...
    private var bottomToolbar: some View {
        HStack(spacing: 14) {
            ...
            // `inflect:` drives the plural ("1 document" / "2 documents"
            // / "0 document"), with the FR plural rules served from
            // Localizable.xcstrings.
            Text("^[\(filteredItems.count) document](inflect: true)")
                .font(.subheadline.weight(.medium))
                .foregroundStyle(.primary)
                .padding(.horizontal, 14)
                .frame(height: 44)
