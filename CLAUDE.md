...
- **Code coverage** : Rust gaté à **90% de lignes** en CI (ratchet — on ne descend plus ; tightening progressif vers 95/98% prévu) via `cargo llvm-cov --workspace --fail-under-lines 90 --summary-only` avec exclusions sur les paths intestables unitairement (entry points, Notion HTTP, extractors Craft/Bear nécessitant fixtures externes). Couverture mesurée 2026-06-02 (après vague 1 + reader.rs réinclus) : **90.16% lines / 88.50% functions / 92.20% regions**. Tests :
  ...
  - 17 dans `crates/realm-codec/tests/reader_new_format.rs` — bytes new-format cluster-tree construits à la main pour exercer `read_table_new` / `collect_strings_new` / `collect_ints_new` / `collect_linklists_new` / `cluster_index_for_col` + paths défensifs. `reader.rs` passé de **31.48% → 63.76%**. Le writer ne produit que de l'old-format, donc impossible de couvrir 100% du reader sans un fichier Craft réel (ou un mini-builder new-format dans le writer — refactor reporté).
  - Pour atteindre 98% : 1069 lignes restantes (ffi.rs imports HTTP non-mockables, retry paths SQLite, use_cases/blocks + db_doc_sync, database_use_cases/query, paths reader cluster-tree non encore exercés).
...
- **`realm-codec/src/reader.rs` à 63.76%** : les 36% restants sont des chemins activés en prod par les fichiers `.realm` Craft mais non couverts par les tests automatiques. **Validation empirique** : 2498 docs / 4224 blocs / 41 skipped lus avec succès sur le fichier Craft du dev le 2026-06-02, le code fonctionne en l'état. **Risque** : si un user importe un Craft avec une topologie atypique (strings longues qui forcent compact-leaf au lieu d'inline-multiply, ou BackLink columns), des paths non testés s'activent — échec gracieux (le reader retourne `vec![]` plutôt que de paniquer), mais data loss silencieuse (blocs skipped sans signal utilisateur). Détails des chemins :
  - `read_compact_string_leaf` (variante 1 du leaf : `[offsets_ref, blob_ref]`, size ≤ 3) — strings concaténées avec offsets
  - `read_perrow_string_refs` + `read_wtype2_string` (variante 2 : width ≥ 16, size > 3) — un ref par row vers un node wtype=2
  - `count_node_rows` avec `is_inner` (old-format B-tree) + `read_cell_btree` — probablement code mort si Craft est 100% cluster-tree, candidat à la suppression plutôt qu'aux tests
  - Bug latent BackLink : `cluster_index_for_col` traite le nibble code 13 comme zero-slot, mais `ColumnType::from_u8(13)` retourne `LinkList` au lieu de `BackLink` (code 14). Cohérence à corriger avant d'écrire un test honnête.
  - **Solution propre** : soit (a) un mini-builder new-format dans `crates/realm-codec/src/write.rs` qui produit le format cluster-tree avec les 3 variantes de string leaf, et tous les tests roundtrip du writer couvrent automatiquement le reader (~2-3h de refactor), soit (b) committer un fichier `.realm` minimal (généré une fois depuis un projet Craft jouet) comme fixture binaire dans `crates/realm-codec/tests/fixtures/` (~50 KB, plus rapide mais moins déterministe).
