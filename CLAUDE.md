...
- **Code coverage** : Rust gaté à **98% de lignes** en CI via `cargo llvm-cov --workspace --fail-under-lines 98 --summary-only`. Couverture mesurée 2026-06-02 : **69.56% lines / 66.57% functions / 69.22% regions** — la CI est actuellement **rouge** et le restera tant que les zones à 0% ne sont pas couvertes : `ffi.rs` (1120 lignes), `extractors/notion/client.rs`, `extractors/notion/mod.rs` (25%), `extractors/craft_combined/mod.rs` (24%), `extractors/craft/mod.rs` (45%), `infrastructure/sqlite_folder_store.rs`, `domain/folder.rs`, `main.rs`, `bin/`. Swift : `xcodebuild -enableCodeCoverage YES` à mesurer quand le job Swift sera réactivé.
