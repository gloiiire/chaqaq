...
use crate::application::use_cases::leaf::find_block_mut;
use crate::domain::leaf::{Block, BlockContent, Leaf};
use uuid::Uuid;

/// Appends a new block to the leaf's top-level block list and persists.
pub fn add_block(
    uow: &dyn UnitOfWork,
    id: Uuid,
...
/// Values: `"ltr"`, `"rtl"`, or `None` to inherit the leaf-level
...
/// Moves a block to a new parent (`None` = leaf root).
///
...
///
/// Returns `InvalidOperation` when the block is already at the leaf root
...
mod tests {
    ...
    impl LeafRepository for MockRepo {
    ...
    fn test_reorder_child_blocks() {
        ...
        let mut doc = Leaf::new(inline("Test"));
    ...
    fn test_move_block_racine_vers_enfant() {
        ...
        move_block(&leaf_uow(&repo), doc.id, child_id, Some(parent_id)).unwrap();
    ...
    fn test_move_block_enfant_vers_racine() {
        ...
        move_block(&leaf_uow(&repo), doc.id, child_id, None).unwrap();
    ...
    fn test_move_block_dans_lui_meme_erreur() {
        ...
        let res = move_block(&leaf_uow(&repo), doc.id, block_id, Some(block_id));
    ...
    fn test_set_block_color_set_and_clear() {
        ...
        set_block_color(&leaf_uow(&repo), doc.id, block_id, Some("red".into())).unwrap();
        let loaded = repo.load(doc.id).unwrap();
        assert_eq!(loaded.blocks[0].color.as_deref(), Some("red"));

        set_block_color(&leaf_uow(&repo), doc.id, block_id, None).unwrap();
        let loaded = repo.load(doc.id).unwrap();
    ...
    fn test_set_block_color_on_nested_child() {
        ...
        set_block_color(&leaf_uow(&repo), doc.id, child_id, Some("blue".into())).unwrap();
        let loaded = repo.load(doc.id).unwrap();
    ...
    fn test_set_block_color_unknown_block_returns_not_found() {
        let repo = MockRepo::new();
        let doc = Leaf::new(inline("Test"));
        repo.save(&doc).unwrap();
        let unknown = Uuid::new_v4();
        let res = set_block_color(&leaf_uow(&repo), doc.id, unknown, Some("red".into()));
    ...
    fn test_indent_moves_block_under_previous_sibling() {
        ...
        indent_block(&leaf_uow(&repo), doc.id, b_id).unwrap();
    ...
    fn test_indent_first_block_fails() {
        ...
        let res = indent_block(&leaf_uow(&repo), doc.id, a_id);
        assert!(matches!(res, Err(PinkhaError::InvalidOperation(_))));
    }
    ...
    fn test_indent_works_inside_nested_subtree() {
        ...
        indent_block(&leaf_uow(&repo), doc.id, child2_id).unwrap();
    ...
    fn test_outdent_moves_block_to_grandparent_after_parent() {
        ...
        outdent_block(&leaf_uow(&repo), doc.id, child_id).unwrap();
    ...
    fn test_outdent_root_block_fails() {
        ...
        let res = outdent_block(&leaf_uow(&repo), doc.id, a_id);
        assert!(matches!(res, Err(PinkhaError::InvalidOperation(_))));
    }
    ...
    fn test_duplicate_root_block_inserts_clone_right_after() {
        ...
        let new_id = duplicate_block(&leaf_uow(&repo), doc.id, a_id).unwrap();
    ...
    fn test_duplicate_deep_clones_children_with_fresh_ids() {
        ...
        let new_id = duplicate_block(&leaf_uow(&repo), doc.id, parent_id).unwrap();
    ...
    fn test_duplicate_nested_block_inserts_at_same_level() {
        ...
        let new_id = duplicate_block(&leaf_uow(&repo), doc.id, c1_id).unwrap();
    ...
    fn test_duplicate_unknown_block_returns_not_found() {
        ...
        repo.save(&doc).unwrap();
        let unknown = Uuid::new_v4();
        let res = duplicate_block(&leaf_uow(&repo), doc.id, unknown);
        assert!(matches!(res, Err(PinkhaError::NotFound(_))));
    }
