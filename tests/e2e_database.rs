use chaqaq::application::database_use_cases::{
    agregat_colonne, ajouter_entree, ajouter_propriete, ajouter_vue, creer_database,
    evaluer_rollups, obtenir_database, requete, requete_groupee,
};
...
fn test_ajouter_propriete_a_database_existante() {
    ...
    assert_eq!(
        obtenir_database(&db_store, db.id).unwrap().proprietes.len(),
        0
    );
...
fn test_flux_rollup_entre_deux_databases() {
    ...
    let db_projets =
        creer_database(&db_store, titre("Projets"), vec![prop_rel, prop_total]).unwrap();
